...
class MailTypeModel(models.Model):
    ...
    @classmethod
    def create(cls, label, template_id="", description="", path="/", verbosity=1):
        try:
...
class NotificationModel(TimeStampModel):
    ...
    template = models.ForeignKey(MailTypeModel, on_delete=models.SET_NULL, blank=True, null=True)
